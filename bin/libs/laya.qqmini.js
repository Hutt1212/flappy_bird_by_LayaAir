window.qqMiniGame = function (exports, Laya) {
    ...
    class MiniFileMgr {
        ...
        static isNetFile(url) {
            return (url.indexOf("http://") != -1 || url.indexOf("https://") != -1) && url.indexOf(QQMiniAdapter.window.qq.env.USER_DATA_PATH) == -1;
        }
        ...
        static read(filePath, encoding = "utf8", callBack = null, readyUrl = "", isSaveFile = false, fileType = "") {
            ...
            MiniFileMgr.fs.readFile({ filePath: fileUrl, encoding: encoding, success: function (data) {
                    ...
                    if (data && readyUrl != "")
                        MiniFileMgr.downFiles(QQMiniAdapter.safeEncodeURI(readyUrl), encoding, callBack, readyUrl, isSaveFile, fileType);
                    else
        ...
        static readFile(filePath, encoding = "utf8", callBack = null, readyUrl = "", isSaveFile = false, fileType = "", isAutoClear = true) {
            ...
            MiniFileMgr.fs.readFile({ filePath: filePath, encoding: encoding, success: function (data) {
                    if ((filePath.indexOf("http://") != -1 || filePath.indexOf("https://") != -1) && filePath.indexOf(QQMiniAdapter.window.qq.env.USER_DATA_PATH) == -1) {
                        if (QQMiniAdapter.AutoCacheDownFile || isSaveFile) {
                            callBack != null && callBack.runWith([0, data]);
        ...
        static downOtherFiles(fileUrl, callBack = null, readyUrl = "", isSaveFile = false, isAutoClear = true) {
            MiniFileMgr.down({ url: fileUrl, success: function (data) {
                    if (data.statusCode === 200) {
                        if ((QQMiniAdapter.autoCacheFile || isSaveFile) && readyUrl.indexOf(".php") == -1) {
        ...
        static copyTOCache(tempFilePath, readyUrl, callBack, encoding = "", isAutoClear = true) {
            ...
            var totalSize = QQMiniAdapter.sizeLimit;
    ...
    MiniFileMgr.down = window.qq.downloadFile;
    ...
    class MiniLoader extends Laya.EventDispatcher {
            ...
            if (!QQMiniAdapter.autoCacheFile) {
                ...
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl)) {
                    if (MiniFileMgr.isNetFile(tempurl)) {
                        MiniFileMgr.downOtherFiles(QQMiniAdapter.safeEncodeURI(tempurl), Laya.Handler.create(MiniLoader, MiniLoader.onDownLoadCallBack, [tempurl, thisLoader]), tempurl);
                    }
                    else {
                        MiniLoader.onDownLoadCallBack(url, thisLoader, 0);
                    }
        ...
        static onDownLoadCallBack(sourceUrl, thisLoader, errorCode, tempFilePath = null) {
            if (!errorCode) {
                ...
                sound.load(QQMiniAdapter.safeEncodeURI(sourceUrl));
        ...
        _loadHttpRequestWhat(url, contentType) {
            ...
            if (Laya.Loader.preLoadedMap[tempurl])
                ...
                if (!QQMiniAdapter.AutoCacheDownFile) {
                    if (MiniFileMgr.isNetFile(tempurl)) {
                    ...
                    if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempurl)) {
                        if (MiniFileMgr.isNetFile(tempurl)) {
                            MiniFileMgr.downFiles(QQMiniAdapter.safeEncodeURI(tempurl), encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [url, contentType, thisLoader]), tempurl, true);
                        }
                        else {
                            MiniFileMgr.readFile(url, encoding, new Laya.Handler(MiniLoader, MiniLoader.onReadNativeCallBack, [url, contentType, thisLoader]), url);
                        }
        ...
        static _transformImgUrl(url, type, thisLoader) {
            ...
            if (!QQMiniAdapter.autoCacheFile) {
                thisLoader._loadImage(QQMiniAdapter.safeEncodeURI(url));
            }
            else {
                var tempUrl = Laya.URL.formatURL(url);
                if (!MiniFileMgr.isLocalNativeFile(url) && !MiniFileMgr.getFileInfo(tempUrl)) {
                    if (MiniFileMgr.isNetFile(tempUrl)) {
                        MiniFileMgr.downOtherFiles(QQMiniAdapter.safeEncodeURI(tempUrl), new Laya.Handler(MiniLoader, MiniLoader.onDownImgCallBack, [url, thisLoader]), tempUrl);
                    }
                    else {
    ...
    class QQMiniAdapter {
        ...
        static init(isPosMsg = false, isSon = false) {
            ...
            if (!QQMiniAdapter.isZiYu) {
                ...
                MiniFileMgr.getCacheList(MiniFileMgr.fileNativeDir, Laya.Handler.create(QQMiniAdapter, QQMiniAdapter.onMkdirCallBack));
            }
            ...
            Laya.HttpRequest._urlEncode = QQMiniAdapter.safeEncodeURI;
        ...
        static sendSinglePicToOpenDataContext(url) {
            ...
            if (fileNativeUrl) {
                QQMiniAdapter.window.qq.postMessage({ url: tempTextureUrl, imgNativeUrl: fileNativeUrl, imgReadyUrl: tempTextureUrl, isLoad: "openJsondatacontextPic" });
            }
            else {
                throw "获取图集的磁盘url路径不存在！";
            }
    ...
    QQMiniAdapter.IGNORE = new RegExp("[-_.!~*'();/?:@&=+$,#%]|[0-9|A-Z|a-z]");
    QQMiniAdapter.safeEncodeURI = function (str) {
        var strTemp = "";
        var length = str.length;
        for (var i = 0; i < length; i++) {
            var word = str[i];
            if (QQMiniAdapter.IGNORE.test(word)) {
                strTemp += word;
            }
            else {
                try {
    ...
    QQMiniAdapter.sizeLimit = (50 * 1024 * 1024);
